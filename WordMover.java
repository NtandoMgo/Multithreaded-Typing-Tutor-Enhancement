...
public class WordMover extends Thread {
	private FallingWord myWord; //private HungryWordMover myHWord;
	...
	public void run() {
		...
		while (!done.get()) {				
			//animate the word
			while (!myWord.dropped() && !done.get() /*&& !myHWord.slided()*/) {
				    myWord.drop(10); //myHWord.slide(10);
					try {
			...
			if (!done.get() && myWord.dropped()/*  && myHWord.slided()*/) {
				score.missedWord();
				myWord.resetWord();
			}
			myWord.resetWord();
